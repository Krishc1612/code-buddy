...
def generate_response(request : List[Dict], sys_prompt : Dict) -> str:
    # request.appendleft(sys_prompt) instead make a separate list
    payload_messages = [sys_prompt] + request #final promt sent to ai 
    ...
    # request.append(output['message']) would have done it if deque was implemented for context window.
...
# Also there could be a maintained "messages" deque of fixed size.
...
# Note: deque is not json serializable.

# The deque concept shall be implemented when we have enough memory resources of preserving context of a chat which here we don't hence we won't be implementing that.
# Also db queries are much faster and safer to rely on.
